...
export default class SplineClimbingWall extends Group {
  ...
  async _init() {
    try {
      ...
      this._canvas.style.opacity = '1'; // Full opacity
      
      // Add to DOM
      document.body.appendChild(this._canvas);
      
      // ADD BLACK BOX TO HIDE SPLINE LOGO 🔥
      this._splineLogoBlocker = document.createElement('div');
      this._splineLogoBlocker.style.position = 'fixed';
      this._splineLogoBlocker.style.bottom = '15px'; // Position higher
      this._splineLogoBlocker.style.right = '5px';   // Position more left
      this._splineLogoBlocker.style.width = '150px'; // Wider coverage
      this._splineLogoBlocker.style.height = '40px'; // Tall enough to cover logo
      this._splineLogoBlocker.style.backgroundColor = '#000000'; // Pure black
      this._splineLogoBlocker.style.zIndex = '10000'; // Above everything
      this._splineLogoBlocker.style.pointerEvents = 'none'; // Don't block interactions
      this._splineLogoBlocker.style.borderRadius = '8px'; // Smooth rounded corners 🔥
      
      // Add the blocker to DOM
      document.body.appendChild(this._splineLogoBlocker);
      
      console.log('🔥 Spline logo blocker added!');
  ...
  dispose() {
    ...
    // Remove Spline logo blocker from DOM 🔥
    if (this._splineLogoBlocker && this._splineLogoBlocker.parentNode) {
      this._splineLogoBlocker.parentNode.removeChild(this._splineLogoBlocker);
    }
